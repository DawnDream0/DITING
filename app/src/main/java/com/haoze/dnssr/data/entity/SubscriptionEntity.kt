...
object SubscriptionKind {
    ...
    private const val LEGACY_BLOCK = "block"
    private const val LEGACY_ALLOW = "allow"
    private const val LEGACY_UNIFIED = "unified"
    private const val LEGACY_ALL = "all"
    private const val LEGACY_REWRITE = "rewrite"

    /** Maps current and legacy stored values onto [DOMAIN] / [HOSTS]. */
    fun normalize(kind: String?): String = when (kind?.trim()?.lowercase()) {
        HOSTS, LEGACY_REWRITE -> HOSTS
        DOMAIN, LEGACY_BLOCK, LEGACY_ALLOW, LEGACY_UNIFIED, LEGACY_ALL -> DOMAIN
        else -> DOMAIN
    }
    ...
    /** True when [kind] is a value this version still writes. */
