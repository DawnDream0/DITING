...
interface BlockRuleDao {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM block_rule WHERE pattern = :pattern AND enabled = 1)")
    suspend fun existsByPattern(pattern: String): Boolean

    @Query("DELETE FROM block_rule_source WHERE source = :source AND ruleId IN (SELECT id FROM block_rule WHERE pattern = :pattern)")
    suspend fun deleteSourceByPattern(source: String, pattern: String): Int

    @Transaction
    suspend fun deleteForSource(source: String, pattern: String): Boolean {
        val deleted = deleteSourceByPattern(source, pattern) > 0
        if (deleted) {
            deleteOrphans()
        }
        return deleted
