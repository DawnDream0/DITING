...
interface AllowRuleDao {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM allow_rule WHERE pattern = :pattern AND enabled = 1)")
    suspend fun existsByPattern(pattern: String): Boolean

    @Query("DELETE FROM allow_rule_source WHERE source = :source AND ruleId IN (SELECT id FROM allow_rule WHERE pattern = :pattern)")
