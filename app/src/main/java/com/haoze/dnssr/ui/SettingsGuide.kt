...
private fun englishGuide(guide: SettingsGuide): SettingsGuide = when (guide.id) {
    ...
    "network_tools" -> guide.copy(
        ...
        message = "Network diagnostics offers four tools: DNS speed test, ping test, DNS lookup, and traceroute.\n\nThe speed test sends real queries for the chosen domain to selected DNS providers and compares their resolution time. The ping test sends ICMP probes to a custom IP or domain and reports latency, packet loss, and TTL. The DNS lookup queries A / AAAA records from the current network's or a custom DNS server and shows resolved IPs, record details such as CNAME, TTL, and the response status. Traceroute probes the path with increasing TTL and shows every router along the way with its address and latency.\n\nAll tools run directly on this device and do not go through the DITING VPN. Results reflect the network state at the moment of execution; some targets limit ICMP or DNS responses, so try different targets or networks when a result looks abnormal."
    )
    "cache" -> guide.copy(
...
object SettingsGuides {
    ...
    val NETWORK_TOOLS = SettingsGuide("network_tools", "网络诊断", "网络诊断集中提供 DNS 查询测速、Ping 测试、DNS 解析查询与路由追踪四类诊断工具。\n\n查询测速向选中的 DNS 服务商发起真实查询，比较各服务商在当前网络中的解析耗时；Ping 测试向自定义 IP 或域名发送 ICMP 探测，输出时延、丢包率与 TTL；DNS 解析查询向当前网络或自定义 DNS 服务器查询域名的 A / AAAA 记录，展示解析 IP 与记录明细；路由追踪通过递增 TTL 的探测展示数据包到达目标所经过的每一跳路由及各跳时延。\n\n所有工具都在设备本机直接执行，不经过谛听 VPN；结果只反映执行时刻的网络状态，部分目标会限制 ICMP 响应或 DNS 查询，结果异常时可更换目标或网络再次验证。")
