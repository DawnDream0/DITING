...
class FloatingLogOverlayController(context: Context) {
    ...
    private fun removeWindow() {
        ...
        logContainer = null
        windowParams = null
        // After a window rebuild the panel restarts from the loading placeholder,
        // so the render cache must be cleared too; otherwise loadLogs skips
        // rendering when the logs are unchanged and the panel stays stuck on
        // "loading logs..."
        lastRenderedLogs = null
        expanded = false
    }
    ...
    private suspend fun loadLogs() {
        ...
        // No new logs: skip rebuilding the whole View tree, avoiding useless
        // main-thread layout/draw passes and View allocations
