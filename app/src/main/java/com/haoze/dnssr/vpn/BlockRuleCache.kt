...
/**
 * In-memory cache of block rules.
 *
 * All enabled rules are loaded into HashSets, giving O(number of domain
 * labels) lookups instead of an O(N) full-table database scan on every DNS
 * query.
 *
 * Matching semantics:
 * - Exact match: domain == pattern
 * - Subdomain suffix match: domain.endsWith(".$pattern"), implemented by
 *   walking every suffix of the domain one label at a time.
 */
...
class BlockRuleCache(
    ...
    fun exportSnapshot(): ExportedBlockSnapshot {
        ...
        val disabled = subscriptionOverrides.filter { it.value == null }.keys.toList()
