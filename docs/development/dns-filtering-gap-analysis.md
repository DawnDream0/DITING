...
解析器位于 `app/src/main/java/com/haoze/dnssr/vpn/AdGuardRuleParser.kt`,被丢弃的规则在导入统计里最多体现为 `unsupportedCount`(正则、`$dnstype` 等走该计数,与 invalid 合并展示为"无效/不支持");`badfilter`/web-only 的 `ignoredCount` 在流式导入路径(`CategorizedRuleStreamImporter`)不累计、完全不可见,用户无感知。逐条证据:
...
| 项 | 改动点 | 要点 |
|---|---|---|
| `$denyallow=` | `AdGuardRuleParser.kt:91-96` 从 web-only 表移出;`ParsedRule` 增加排除域字段;`policy_snapshot.go` 规则条目支持"命中排除域则跳过";快照 JSON 与 `DomainPolicyEngine.buildRuleSnapshotJson` 同步 | AdGuard DNS 语义:denyallow 域做后缀匹配。对激进型列表(如 `||*^$denyallow=…`)影响大 |
| 正则规则 `/…/` | 解析产出 RegexRule;两侧各加 regex 匹配器(Kotlin `WildcardPattern` 旁、Go `wildcardMatcher` 旁);数量少,走内存列表通道(参照 DTRI 编译时跳过 wildcard 的既有模式,`MappedSubscriptionRuleIndex.kt:204`) | AdGuard 语义:正则匹配完整域名,不自动后缀。Kotlin 侧编译失败计数 invalid |
| `$dnstype=` | 解析保留;Go 侧在 `serveDNS` 增加 (pattern, qtype) 匹配 | 典型用途 `$dnstype=AAAA` 禁 IPv6 解析 |
...
| 正则 `/…/` | 支持 | **丢弃** | 有(P1) |
| `$dnstype=` | 支持 | **丢弃** | 有(P1) |
| CNAME 链逐级过滤 | 支持(cloaking 防护) | **无** | 有(P0-A,主因) |
| SVCB/HTTPS(65) TargetName 过滤 | 支持 | **无** | 有(P0-A) |
